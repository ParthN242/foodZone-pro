...
import { createFoodItemAsync } from "../../../Redux/Food/Food/foodApi";
...
const AddFood = () => {
  ...
  const submitHandler = (e) => {
    ...
    toast.info("Saving Food Item...", { icon: false, autoClose: 1000 });
    dispatch(createFoodItemAsync(data));
  };
  // Clear Form
  const clearForm = () => {
    setTitle("");
  ...
  useEffect(() => {
    if (success) {
      toast.dismiss();
      toast.success("Food Item Saved Succefully");
      clearForm();
      dispatch(clearState());
    }
    if (error) {
  ...
  return (
    <section className="flex flex-col max-h-screen">
      ...
      <div className="px-8 py-4 overflow-auto cartScrollbar">
        {/* Form */}
        <form
